...
import { doc, setDoc, updateDoc, getDoc, collection, query, where, limit, getDocs } from 'firebase/firestore';
...
app.get('/users/:userId/submissions', async (req, res) => {
  try {
    ...
    const submissions = querySnapshot.docs
      .map((doc) => ({ submissionId: doc.id, ...doc.data() }))
      .sort((a, b) => {
        const ta = a.createdAt?.toMillis?.() ?? new Date(a.submittedAt || 0).getTime();
        const tb = b.createdAt?.toMillis?.() ?? new Date(b.submittedAt || 0).getTime();
        return tb - ta;
      });
...
app.get('/api/problems', async (req, res) => {
  try {
    const snapshot = await getDocs(collection(db, 'problem'));
    const problems = snapshot.docs.map((d) => ({
      id: d.id,
    ...
    res.status(500).json(errorResponse(500, 'Failed to list problems'));
  }
});
...
app.put('/api/problems/:problemId', async (req, res) => {
  try {
    const { problemId } = req.params;
    const { Adminsrc, Inputname, Remaining, Testcases, Timeout,
            title, description, difficulty, examples, constraints, tags } = req.body;
...
/**
 * Pool benchmark — queues a minimal job and polls Redis for result
 * POST /api/benchmark/pool
 */
app.post('/api/benchmark/pool', async (req, res) => {
  const submissionId = `bench_pool_${Date.now()}_${uuidv4().slice(0, 8)}`;
  ...
  try {
    await setInRedis(`submission:${submissionId}:status`, 'queued', 60);
    await sendMessage(job);

    // Poll Redis until result arrives (max 10s, 20ms interval for accuracy)
    const result = await new Promise((resolve) => {
      let attempts = 0;
      const iv = setInterval(async () => {
        const r = await getFromRedis(`submission:${submissionId}:result`);
        if (r || ++attempts > 500) {
          clearInterval(iv);
    ...
    res.json(successResponse({ totalTime, status: result?.status }));
  } catch (error) {
...
/**
 * Sequential benchmark — same execution as pool but enforced one-at-a-time
 * via a server-side mutex. Simulates a system with no parallelism.
 * POST /api/benchmark/sequential
 */
let sequentialLock = Promise.resolve();
app.post('/api/benchmark/sequential', async (req, res) => {
  // Chain onto the existing lock so jobs run one after another
  const result = await (sequentialLock = sequentialLock.then(async () => {
    const submissionId = `bench_seq_${Date.now()}_${uuidv4().slice(0, 8)}`;
    const startTime = Date.now();
    const job = {
      submissionId,
      problemId: 'benchmark',
    ...
    const r = await new Promise((resolve) => {
      let attempts = 0;
      const iv = setInterval(async () => {
        const cached = await getFromRedis(`submission:${submissionId}:result`);
        if (cached || ++attempts > 500) { clearInterval(iv); resolve(cached); }
      }, 20);
    });
    return { totalTime: Date.now() - startTime, status: r ? JSON.parse(r)?.status : null };
  }));
